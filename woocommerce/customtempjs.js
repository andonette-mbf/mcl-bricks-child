jQuery(document).ready(function () {
	...
	jQuery(
		'.fields select:not([name="menu-890"]):not([name="menu-889"]):not(#course), .form-row select, .filter-container .sort-container select, .people-select-menu select, .woocommerce #payment .form-row select, .woocommerce-page #payment .form-row select'
	).selectize();
	...
	jQuery(".fields select#course").selectize({
		render: {
			option: function (data, escape) {
				return (
					"<div class='option' data-locations='" +
					escape(data.locations) +
					"'>" +
					escape(data.text) +
					"</div>"
				);
	...
	jQuery(".wpcf7-submit, form#commentform input#submit").addClass(
		"cta-button"
	);
	...
	jQuery('iframe[src*="youtube"]').wrap(
	...
	jQuery(".accreditation-list .accredications-list-inner").slick({
		...
		slidesToScroll: 1,
		prevArrow:
			'<button type="button" class="fal previous fa-chevron-left general">',
		nextArrow:
			'<button type="button" class="fal next fa-chevron-right general">',
		dots: false,
		arrows: false,
		autoplay: true,
		autoplaySpeed: 2000,
	...
	jQuery(
		".header li.menu-item-has-children a, .header ul.menu > li > ul.sub-menu"
	).mouseleave(function () {
	...
	jQuery("ul#menu-mobile-menu li.menu-item-has-children > a").click(function (
		e
	) {
		e.preventDefault();
	...
	jQuery(".scollingToSection").click(function (e) {
		...
		jQuery("html, body").animate(
			{
				scrollTop:
					jQuery("#scroll" + scrollingModule).offset().top - 70,
			},
			2000
		);
	});
	...
	jQuery(".wpcf7").append(
		'<div class="form-overlay"><div class="lds-ring vhboth"><div></div><div></div><div></div><div></div></div></div>'
	);
	...
	jQuery(
		".standard-post a[href*='.png'],.standard-post a[href*='.jpg'],.standard-post a[href*='.gif']"
	).attr("data-lightbox", "postImages");
	...
	jQuery(".tab-content > .tab-pane:nth-child(1)").addClass(
		"fade active show"
	);
	...
	jQuery(".related-products-standard").slick({
		...
		slidesToScroll: 1,
		prevArrow:
			'<button type="button" class="fal previous fa-long-arrow-left general">',
		nextArrow:
			'<button type="button" class="fal next fa-long-arrow-right general">',
		dots: false,
		arrows: true,
		responsive: [
			{
	...
	//custom partner tabs
	jQuery(".row.logos-top article:nth-child(1) a.partner-logo").addClass(
		"active"
	);
	...
	//custom partner tabs
	jQuery(".partners-global-block .partner-quotes-slider").on(
		"afterChange",
		function (event, slick, currentSlide, nextSlide) {
			var SlideNo = currentSlide + 1;
			jQuery(".row.logos-top a.partner-logo").removeClass("active");
			jQuery(
				".row.logos-top a.partner-logo[data-partner=" + SlideNo + "]"
			).addClass("active");
		}
	);
	...
	jQuery.fn.multiply = function (numCopies) {
		var newElements = this.clone().html(
			this.clone().html().replace(/{X}/g, 1)
		);
	...
	jQuery(".step-field.person-field a").click(function (e) {
		...
		if (
			jQuery(".training-course-steps .course-step#step-4").is(":hidden")
		) {
		...
		jQuery(".container-fluid.training-course-product").attr(
		...
		jQuery(
			".training-sidebar .sidebar-selections p.from-price.price"
		).hide();
		jQuery(
			".training-sidebar .sidebar-selections p.total-price.price"
		).show();
		jQuery(
		...
		var spaces_remaining = jQuery(
			".container-fluid.training-course-product"
		...
		jQuery("div#delegate-details .outputted-fields").html(
			jQuery("div#delegate-details .step-inputs-split").multiply(
				personNumber
			)
		);
	});
	...
	jQuery(".toggle-style-block a.course-style").click(function (e) {
		e.preventDefault();
		jQuery(
			".toggle-style-block a.course-style span.toggle-label"
		).toggleClass("active");
		jQuery(
	...
	jQuery("#wc-bookings-booking-form > fieldset").on(
		...
		function (event, fdate) {
			...
			if (year == 2023) {
				...
				jQuery("input#changed-cost-of-course").attr(
					"value",
					price_cost
				);
			...
			if (
				jQuery(".training-course-steps .course-step#step-3").is(
					":hidden"
				)
			) {
				jQuery(
					".training-course-steps .course-step#step-3"
				).slideDown();
			}
	...
	jQuery(".table-section a.book-now-button").click(function (e) {
		...
		if (
			jQuery(".training-course-steps .course-step#step-3").is(":hidden")
		) {
	...
	jQuery("a#confirm-boooking").click(function (e) {
		...
		if (missing_delegate_info.length > 0) {
			...
			jQuery("html, body").animate(
				{
					scrollTop:
						jQuery("#delegate_info_error").offset().top - 250,
				},
				2000
			);
			return false;
	...
	jQuery(".step-field.qualification-field a").click(function (e) {
		...
		if (classNotFound == false) {
			if (
				jQuery(".training-course-steps .course-step#step-5").is(
					":hidden"
				)
			) {
				jQuery(
					".training-course-steps .course-step#step-5"
				).slideDown();
			}
			...
			if (
				jQuery(".training-course-steps .course-step#step-5").not(
					":hidden"
				)
			) {
	...
	jQuery(".reviews-title-block a.leave-review").click(function (e) {
		e.preventDefault();
		jQuery(
			".reviews-product-block .woocommerce-tabs.reviews-form div#review_form_wrapper"
		).slideToggle();
		jQuery(
			".reviews-product-block .woocommerce-tabs.reviews-form div#comments"
		).slideToggle();
	});
	...
	jQuery(".row.title-row.step-title a.previous-step").click(function (e) {
		...
		jQuery("html, body").animate(
			{
				scrollTop:
					jQuery(
						".training-course-steps .course-step#step-" +
							previousStep
					).offset().top - 160,
			},
			2000
		...
		jQuery(
			".training-course-steps .course-step#step-" + currentStep
		).slideUp();
	});
	...
	jQuery(
		".header .menu-main-menu-container > ul > li#menu-item-1484 > a"
	).click(function (e) {
		e.preventDefault();
	...
	jQuery("body").on(
		"input",
		"div#delegate-details .delegate-fields .outputted-fields .step-inputs-split input.delegate_NI",
		function (e) {
			var fieldNumber = jQuery(this).attr("data-number");
			var fieldValue = jQuery(this).val();
...
jQuery(window).load(function () {
	...
	jQuery("form.slider-form:not(.new-slider-style) .fields select#course").on(
		"change",
		function (event) {
			jQuery(
				"form.slider-form:not(.new-slider-style) .fields select#location"
			).removeClass("show-humber");
			jQuery(
				"form.slider-form:not(.new-slider-style) .fields select#location"
			).removeClass("show-london");
			jQuery(
				"form.slider-form:not(.new-slider-style) .fields select#location"
			).removeClass("show-nottingham");
			jQuery(".location-select").addClass("disabled");
			var selectedValue = jQuery(this).val();
			var $selectedOption =
				jQuery(this)[0].selectize.getOption(selectedValue);
			if (
				$selectedOption.data("locations").indexOf("lincolnshire") >= 0
			) {
				jQuery(".location-select").removeClass("disabled");
				jQuery(
					"form.slider-form:not(.new-slider-style) .fields select#location"
				).addClass("show-humber");
			}
			if ($selectedOption.data("locations").indexOf("london") >= 0) {
				jQuery(".location-select").removeClass("disabled");
				jQuery(
					"form.slider-form:not(.new-slider-style) .fields select#location"
				).addClass("show-london");
			}
			if ($selectedOption.data("locations").indexOf("nottingham") >= 0) {
				jQuery(".location-select").removeClass("disabled");
				jQuery(
					"form.slider-form:not(.new-slider-style) .fields select#location"
				).addClass("show-nottingham");
			}
		}
	);
});
